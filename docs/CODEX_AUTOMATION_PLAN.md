...
| M7-04 | 第二台电脑与 CF 接入复核 | 真实客户端 HTTPS；用户提供隧道后完成公网项，未提供则写待验收 |
...
| M8-02 | `doctor.ps1`、启动/停止、局域网 HTTPS 和 CF 说明 | 干净目录可部署；数据与程序分离；不默认打开公网端口 |
...
2. 第二台电脑可通过 HTTPS 登录，真实 NX 和真实 TIA 均可独立和双屏操作。
3. UG 绘图区/完整界面切换、核心视图操作、建模编辑与本地文件流程均经实测。
4. 博图工程编辑、编译、保存和相关弹窗经过真实工作流验收。
5. 安全、输入准确性、断线恢复、保存保护和 LAN 性能的所有必需项通过。
6. 完成 8 小时耐久测试，报告包含采样数据、故障和资源变化。
7. 用户提供 CF 入口后，CF 公网项实测通过；未提供时只能标记“公网待验证”。
