...
2026-09-24 LC-08 范围补充：对用户新增的“NX 自身最小化仍实时出画且宿主鼠标自由”要求，已固定的 Sunshine v2026.516 Windows 源码使用 `SendInput` 发送鼠标/键盘，仍是宿主会话的一套系统输入；Sunshine + Moonlight Web 即使配对也不能凭同桌面串流满足独立鼠标。该架构结论不等同于本候选的完整配对/NX 延迟 A/B 已测试；原 LC-06 待测项保留。其他独立会话 GitHub 候选、当前 Home 条件及真实 NX 最小化 WGC/PrintWindow 负例见 [LC-08 现成方案试验](./LC08_OPEN_SOURCE_TRIAL_20260924.md)。
