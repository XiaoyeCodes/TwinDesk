...
上一同步检查点位于 main，提交为 9b3e8b9ef4a51f31c59417df52ba1678483cbc70，标题 Improve local input scheduling and prepare isolated streaming comparison。包含队列/统计/回归测试、第三方便携包锁定与准备/启停脚本、证据范围及续接文档。2026-09-24 的最新检查点标题为 Instrument local media and native input latency stages，父提交为 9b3e8b9；包含新增分段计时、175项C#/63项JS回归和本机有限实测记录。接收端以 git log -1 与 origin/main 核对最新标题或后续提交；原始 artifacts 仍不随源码同步。
...
2026-09-24 续接：新增 LC01_MEDIA_TIMING_20260924.md，已验证 C#175/JS63 以及两个本机浏览器媒体有限实测（300帧生成图、31帧自有WGC窗口）。原生输入检查/发送、源轮询、编码驻留、浏览器解码/绘制已有分段计时，真实 NX 可见响应仍缺。此轮观察 NX 是最小化的 `070.prt（只读）`，不同于9月4日历史记录；未恢复或保存。下一项先准备可写隔离工程并进行实体键鼠 + NX 的完整闭环采样，再依据开销决定 LC-04/05，不重复已有逻辑/媒体测试。当前用户再次确认仍是同一台电脑浏览器控制本机 NX，“局域网内”不代表第二台电脑产品范围。

2026-09-24 再续接（覆盖上段未完成项）：`LC01_NX_INTERACTION_20260924.md` 记录了可写隔离 `070-editable.prt` 原生打开、三轮同机实体滚轮/中键与真实可见视图变化、用户延迟反馈、旧 F12 误报及修复后的两轮 STOPPED。新 LC-04 候选使用逐次进程窗口枚举和 `ProcessIdToSessionId`，只读 NX 微基准均逐次对比一致，C#176/JS64 通过；**尚无优化后实体复测**。服务已停止，NX 仍打开且标题出现未保存内存修改，不得强制关闭/保存。下一项用同一隔离副本恢复服务，做候选优化 A/B 与 LC-05 动态画面间隔诊断，继续 SC07/SC08 门槛；上述旧“真实 NX 缺”是本日较早历史状态。
...
最新下一项按本文件上方 2026-09-24 再续接和 PROJECT_STATUS：LC-04 优化后实体 NX A/B、LC-05 动态媒体间隔定位。此前有序移动合并、SPSC物理桥与分段统计已实现；LC-06 独立开源便携环境启动/停止已验证，但同机输入与 NX A/B 仍待，见 OPEN_SOURCE_COMPARISON.md；不要从零重写已完成公共部分。
