...
LC-05 可在 H.264 媒体探针启动参数末尾显式加 `--60fps`，用于本机 60 FPS 候选对照；网页原有时长选项不变，服务按实际配置换算编码帧数，报告记录真实帧数和间隔。默认仍为30 FPS，`--jpeg --60fps` 会拒绝。生成图和自有动态原生窗口的有限验证不能证明真实 NX 的 60 FPS、输入可见延迟或长时稳定性。
NX 本机接管脚本对应使用 `./scripts/start-local-nx-console.ps1 -HighFrameRate`；先在 NX 原生窗口核对已打开的是脚本所指向的可写隔离副本，F12 可退出接管。
