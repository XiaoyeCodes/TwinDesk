...
## 两个真实 NX 的同页媒体试验（2026-09-24）

两个 `ugraf.exe` 进程均已启动且窗口**没有最小化**时，先用 `Workbench.Probe --process ugraf --list` 取得两个当前 HWND，核对其 PID 不同；随后可在不影响现有 8091 的 8093 端口运行：

```powershell
. ./scripts/environment.ps1
& $Dotnet run --project tools/Workbench.MediaProbe -c Release --no-restore -- --process ugraf --window <左侧当前 HWND> --owned --dual-nx-window <右侧当前 HWND> --port 8093
```

打开 `http://127.0.0.1:8093/`，页面自动连接左右两条真实 NX WGC/H.264/WebCodecs 流。启动参数拒绝相同 HWND/PID、非 NX 首目标、最小化目标及任何 `--input-*`/`--local-console` 混用；两侧**媒体通道没有控制端点**，独立的同桌面穿透试验见下文。每路捕获/编码按自身绑定身份和场景独立进行；关掉页面释放两路连接，Ctrl+C 停止此探针。默认 720p/30 FPS，仍可显式用 `--1080p` 或 `--60fps` 作媒体实验，但双路资源与长期性能没有通过验收。不能把两个画面显示、浏览器鼠标在网页移动、或右侧原生菜单由其他工具打开的结果当作浏览器键鼠控制通过。

当前正式输入前校验仍要求目标 NX 是 Windows 前台，且系统命中测试落在 NX 窗口。浏览器盖住 NX 时，`FOCUS_DENIED`/`POINTER_TARGET_DENIED` 是预期保护；不要为演示删除它们。双路自由跨栏的原生输入仍需要独立验证的受控焦点/指针方案，并证明右键、拖动、键盘、菜单/对话框、失焦释放和关闭重建都不误投到另一实例。
...
默认1280×720；显式 `--1080p` 或两个启动脚本的 `-FullHd` 选择1920×1080，可与JPEG组合。H.264 的 1080p 档配置目标码率 10 Mbps，720p 仍为 4 Mbps；这是编码器配置，不代表实际产生的瞬时码率。页面、协议、解码帧、场景和报告都严格核对档位；本机体验页按档位在可用视口内显示原始画布宽度。输出1080p不代表源窗口有1080p细节：请先把 NX 原生窗口放大到接近显示器尺寸，再比较文字清晰度和实体操作延迟。更高清可能增加编码/解码延迟，不是性能达标声明。服务已运行时切换档位需正常退出旧服务，再用 `./scripts/start-local-nx-console.ps1 -HighFrameRate -FullHd` 重新启动；去掉 `-FullHd` 可回到原720p低延迟档。
...
`--process ugraf --window <左侧根 HWND> --owned --dual-nx-window <右侧根 HWND> --port 8093` 绑定两个不同进程的真实 NX 窗口。网页根页是简约双栏控制台；两路真实 WGC/H.264/WebCodecs 画面自动连接，首帧后才显示在线，画面状态由各自 iframe 的实际呈现帧通知。8091 原单窗模式不受影响。

跨栏输入是**同一个 Windows 桌面的本机试验**，不使用后台 `PostMessage` 或合成软件界面。用户须在独立 Chrome/Edge **应用窗口**打开 `http://127.0.0.1:8093/`，两个 NX 都保持可见且非最小化。普通浏览器标签页及 Codex 内置浏览器只提供画面，控制按钮禁用。可以在 Windows“运行”中手动执行 `chrome.exe --app=http://127.0.0.1:8093/`（或 `msedge.exe --app=http://127.0.0.1:8093/`）；不要将 Codex 本身作为覆盖窗口。此步骤未由代码自动启动浏览器。

点击“开始跨栏控制”后，服务核对浏览器前台/唯一窗口、NX 进程和会话、浏览器内容区与 CSS 比例，保存原生窗口布局，将两个 NX 调整到视频栏的物理坐标，再使**这个独立浏览器窗口**保持可见且鼠标可穿透。实体鼠标直接命中下方真实 NX；服务轮询指针所在栏并尝试让该 NX 成为键盘前台。Windows 可能限制悬停切焦，遇到这种情况单击目标画面切换键盘焦点。状态栏只在实际前台为目标 NX 时显示“键盘目标”。F12、窗口关闭/最小化、锁屏或布局偏移停止试验并恢复窗口样式/位置；无固定 7 秒或其他使用时长上限。窗口样式由本进程恢复，因此不要强杀 8093 服务；若进程崩溃而浏览器仍可见却不能点击，关闭该独立浏览器应用窗口即可撤销其样式。

这是实体鼠标穿透的同机方案，浏览器负责可见视频，并没有在网页里逐事件转发鼠标；顶部按钮在接管期间不可点，使用 F12 退出。需要两窗口完全对齐和真实 NX 菜单、对话框、拖动、跨栏键盘以及视频继续更新的实测，代码构建或两路视频在线**不等于**跨栏操作通过。最小化 NX、其他电脑/独立会话、正式鉴权与 8 小时工作流仍不在该局部试验已验证范围。
